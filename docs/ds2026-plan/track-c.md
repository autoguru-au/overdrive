...
>
> **Extension — tenant-variant namespace keys are also retained.** A named legacy namespace key is repointable only if NO theme — including the MFE tenant themes — overrides it to a value that diverges from the (unbridged) semantic target. Known tenant-variant key: **`colours.background.light`** (smartFleet + ampol override it to their custom `gray['100']`, while `color.background.emphasisInactive` is raw base `#eef0f2`) — RETAINED wherever found (first hit: AutoSuggest C-P8, corrected in review; also retained on BulletText `:16` in C-P4). **`colours.background.neutral`** is **theme-variant** too: the in-repo `neutral` theme overrides it to `gray400` while `color.background.inactive` is raw base `gray300` → RETAINED (Stack C-P4). Before any namespace repoint, check the tenant theme packages (`mfe/packages/themes/*/lib/*/tokens.ts`), not just the in-repo themes. Verified still-safe today: `background.body`, `foreground.body`, `typography.colour.{dark,neutral,muted}` (no theme/tenant overrides these three — C-P4 repointed the default text path onto `color.foreground.{primary,secondary,tertiaryInactive}`), `border.colours.gray` (no tenant/in-repo divergence).
...
| **C-P9** | ~~Shared style utilities repoint~~ **SUPERSEDED — retained until the major** (intent rule: both utilities are intent-derived/theme-overridden; see the C-P9 work order's supersession note). W3c-P1's C-P9 dependency dissolved. | — (no code ships) |
| _(folded)_ | Repoints for components with a Wave-3 restyle happen inside W3a/3b/3c (not a C-Pn). | C-P1 + paired W3 pkg |
| **C-theme-bridge** ✅ | Corrective package (Deviation 12): aliases semantic `color.gamut` onto legacy `colours.gamut` CSS vars so tenant/flat_red theme overrides drive the semantic gamut too; reverts 4 intent-derived swaps to legacy pending the major. Landed `90b884ab`. | C-P1..C-P8 (found while verifying their per-theme output) |
| **F-1 / F-2** ✅ | Folded-purge packages (Deviation 14, previously undocumented here): repoint the **safe internals** of components with a *pending* Wave-3 fold (CheckBox/Radio/Switch/CheckableBase/InputBase/Button/Tabs/Tooltip/Pagination/ProgressSpinner/Alert), leaving intent/theme-variant/public-API refs for the fold itself. Landed `af61bd14`. | C-P1 |
...
| **Switch** | `Switch.css.ts:9-12` | `colours.foreground.body`, `colours.background.{body,neutral,light}` | `color.foreground.primary`, `color.background.default`, `color.background.inactive`, `color.background.emphasisInactive` | N | ✅ | **W3a-P4** | folded 3a · **F-1 ✅** (safe internals landed `af61bd14`; full fold pending) |
| **Slider** | `Slider.css.ts:59,62,80,97` | `colours.gamut.{gray300,gray200,gray900,gray500}` | `color.gamut.gray['{300,200,900,500}']` | G | n/a | **W3b-P6** | folded 3b (not yet landed) |
| **BulletText** | `BulletText.css.ts:12,16,20,24` | `colours.intent.primary.background.{mild(green200),standard(green600)}`, `colours.background.light`(gray200), `typography.colour.dark` | **DONE (C-P4):** `:24` `typography.colour.dark`→`color.foreground.primary`. **RETAINED:** `:12,20` `colours.intent.primary.*` (intent rule), `:16` `colours.background.light` (tenant-variant — smartFleet/ampol → gray100) | G/N | ✅ | standalone | **C-P4 ✅** |
| **BulletList** | `Bullet.css.ts:23,35,42` | `typography.colour.dark` | **DONE (C-P4):** `typography.colour.dark`→`color.foreground.primary` (×3) | N | ✅ | standalone | **C-P4 ✅** |
| **TextLink** | `TextLink.css.ts` legacy `root`/`muted`; `TextLink.tsx` legacy `<Text colour>` | `typography.colour.link`(green600); JSX `colour` | `color.interactive.link`(green600) / **`color.link.*`** (landed — *not* `color.button.linkedText.*`, which never existed) | N | ✅ (interactive.link) | **W3c-P2** | **landed (AG-20713):** both paths consume `color.link.*`. The new `variant` (linked-text) path reads the full ramp; the legacy path's label, resting underline and `muted` hover flood moved onto `color.link.primary`, flipping the default link colour green600 `#01C68C` → green800 `#18856F` (2.22:1 → 4.54:1 on white, so an AA fix rather than a restyle). Shipped as a **major** accordingly. Line refs were stale; see `docs/ds2026-specs/TextLink.md`. |
| **ScrollPane** | `ScrollPane.css.ts:6,20,28` | `colours.gamut.{gray300,gray400}` | **DONE (C-P4):** `colours.gamut.gray300`→`color.gamut.gray['300']` (`:6,20`), `gray400`→`color.gamut.gray['400']` (`:28`) — gamut bridge-safe | G | n/a | standalone | **C-P4 ✅** |
| **Radio** | `Radio.css.ts:9,10,11` | `colours.foreground.body`, `colours.background.{body,neutral}` | `color.foreground.primary`, `color.background.default`, `color.background.inactive` | N | ✅ | **W3a-P3** | folded 3a · **F-1 ✅** (safe internals landed `af61bd14`; full fold pending) |
| **CheckBox** | `CheckBox.css.ts:9,10,11` | same as Radio | same | N | ✅ | **W3a-P2** | folded 3a · **F-1 ✅** (safe internals landed `af61bd14`; full fold pending) |
| **Button** | `Button.css.ts:11,217,335`; `Button.tsx:84` | `colours.intent` (all), `typography.colour.{neutral(gray700),secondary(gray700)}`; ProgressSpinner `colour` | `color.button.*` (W1-P2) + `color.foreground.secondary`(gray700) | N | per-intent | **W3a-P1** | folded 3a · **F-1+F-2 ✅** (safe internals landed `af61bd14`; full fold pending) |
...
| **Stepper** | `Stepper.css.ts:17,20`; `.tsx:66,68,162,189` | `colours.intent.primary.background.strong`(green700); JSX `colour`/`backgroundColour`/`borderColour` | `color.gamut.green['700']`; JSX→semantic | G/N | n/a | **W3b-P6** | folded 3b (not yet landed) |
| **StarRating** | `StarRating.css.ts:7,10` | `colours.intent.shine.{foreground(yellow500),background.standard(gray200)}` | `color.gamut.yellow['500']`, `color.gamut.gray['200']` | G | n/a | **W3a-P5** | folded 3a (not yet landed) |
...
| **Alert** | `Alert.tsx:13,51,57,59,80,99` | `sprinklesLegacyText({color:intent})`, JSX `backgroundColour="white"`/`borderColour="gray"`/`colour` | `color.info/success/warning/alert.*` (value-split); JSX→`color.background.default`/`color.border.default` | N | drift (intents) | **W3c-P7** | folded 3c · **F-2 ✅** (safe internals landed `af61bd14`; full fold pending) |
| **Stack** | `Stack.css.ts:13` | `colours.background.neutral`(gray300) | **RETAINED (C-P4, major-only):** the `neutral` in-repo theme overrides `colours.background.neutral`→gray400, whereas semantic `color.background.inactive` is raw base gray300 (unbridged) → repoint would drift under `neutral`. Left on legacy. | N | ✅→drift | standalone | **C-P4 ✅ (retained)** |
| **EditableText** | `EditableText.css.ts:11`; `.tsx:182` | `typography.colour.muted`(gray400); JSX `colour` | **DONE (C-P4):** `:11` `typography.colour.muted`→`color.foreground.tertiaryInactive` (gray400, un-overridden in all themes/tenants). **RETAINED:** `.tsx:182` `colour={colour}` — pass-through of the public union-typed `colour` prop (intent-capable) → intent/union rule. | N | ✅ | standalone | **C-P4 ✅ (tsx retained)** |
| **DropDown** | `DropDownOption.css.ts:10`; `.tsx:33` | `colours.background.light`(gray200); JSX via `textStyles({colour})` | `color.background.emphasisInactive`; textStyles→`color` | N | ✅ | **W3b-P3** | folded 3b (not yet landed) |
...
| **Calendar** | `Calendar.css.ts:43,44,48,51,52,55,58,59,86,87,91,94,97,99,112` (+`:98` straggler) | `colours.background.body`, `colours.foreground.body`, `colours.gamut.{gray200,gray400,gray600,gray300}`; `border.colours.light` (`:98`, missed by the C-P3 grep — `border.colours.*` isn't caught by the root-level `tokens.colours.*` lint selector) | `color.background.default`, `color.foreground.primary`, `color.gamut.gray['*']`; `:98`→`color.interactive.borderMuted` (gray200 in all themes; gamut gray200 would drift under `flat_red` `#CFD8DC`) | G/N | ✅ | standalone | **C-P3 ✅** (straggler `:98` repointed on `feature/calendar-new-tokens`, incl. radius `md`→`small`) |
...
| **Tooltip** | `useTooltip.tsx:144,149` | `backgroundColour="gray900"`, `colour="white"` | `backgroundColor=gamut.gray900`, `color=fg.reverse` | G/N | n/a | **W3c-P3** | **F-2 ✅ (fully repointed — folded 3c-P3 target superseded)** |
| **Badge** | `Badge.css.ts` `colour` is a **recipe variant NAME**, not a sprinkles prop — **false positive**; Badge renders via its own recipe. Repoint the recipe's colour values in **W3c-P1**. | recipe variants | recipe colour values → gamut/semantic | N | — | **W3c-P1** | folded 3c (not yet landed) |
...
| **C-P4** | BulletText (4), BulletList (3), ScrollPane (3), Stack (1), EditableText (1), **`styles/typography.ts` default path** | Text-flow + the library-wide default text-colour repoint (`typography()`/`textStyles()`). Highest **leverage** (touches every Text consumer) → `opus` Builder, extra Chromatic breadth. **DEFAULT PATH REPOINTED ✅** — see decision note below. `Text.tsx`/`Heading.tsx` component defaults left untouched (out of C-P4 file scope; pixel-identical either way). | 12 + text path |
...
> **C-P4 default-path decision (repointed — recorded 2026-07-07).** The per-key theme-variance check was run for the two default-path keys (`typography.colour.dark`, `typography.colour.neutral`) plus `muted` (EditableText), across **base, flat_red, neutral (in-repo) and all MFE tenants (smartFleet, ampol, fleetGuru; dynamicGuru has no `tokens.ts`)**:
>
> | key | base | flat_red | neutral | smartFleet | ampol | fleetGuru | semantic target | target value (all themes) | equal? |
> |---|---|---|---|---|---|---|---|---|---|
> | `typography.colour.dark` | gray900 `#212338` | inherits base | gray900 `#212338` | inherits (flatRed→base) | inherits | inherits (base) | `color.foreground.primary` | raw gray900 `#212338` (unbridged, no theme overrides `color.*`) | ✅ everywhere |
> | `typography.colour.neutral` | gray700 `#484c5f` | inherits base | gray700 `#484c5f` | inherits | inherits | inherits | `color.foreground.secondary` | raw gray700 `#484c5f` | ✅ everywhere |
> | `typography.colour.muted` | gray400 `#8f95a1` | inherits base | gray400 `#8f95a1` | inherits | inherits | inherits | `color.foreground.tertiaryInactive` | raw gray400 `#8f95a1` | ✅ everywhere |
>
> No theme overrides `dark`/`neutral`/`muted` (they only override `primary`/`link`/`secondary`/`information`), and no theme touches semantic `color.foreground.*` — so the three keys are value-identical to their semantic targets in every theme. **Decision: repoint the default path.** `typography()` now routes its default colour through the semantic `sprinkles({color})` path — body default `neutral`→`secondary`, `strong`/heading default `dark`→`primary`; an explicitly-supplied legacy `colour` still resolves through `sprinklesLegacyText` (intent keys are theme-overridden — NEVER repointed). `sprinklesLegacyText` and the legacy `colour` arg of `textStyles` remain exported/functional (Alert still uses them). `DEFAULT_TEXT_COLOUR` stays exported; new `DEFAULT_TEXT_COLOR`/`DEFAULT_HEADING_COLOR` semantic constants added.
>
...
> **⛔ SUPERSEDED BY THE INTENT RULE (orchestrator ruling, post-theme-bridge — no code ships for C-P9).** Both utilities are intent-derived machinery and are **RETAINED on the legacy contract until the major**: `intentColorset.css.ts` reads `tokens.colours.intent.*` for all 9 intents — repointing any chromatic entry to a fixed gamut grade would freeze base values and silently drop tenant/runtime intent overrides (e.g. Badge `primary` under smartFleet would turn base-green instead of tenant purple); `focusOutline.css.ts` reads `colours.foreground.link`, which is theme-overridden (neutral → blue500, fleetGuru → yellow500) and therefore intent-like. The per-intent measured table below is preserved as the **W3c/W4 reference data** — the intent→semantic mapping (with its value-split + Golden-rule-exception evidence) is the job of the opt-in W3c restyles (Badge W3c-P1, Alert W3c-P7) and the major. **W3c-P1's dependency on C-P9 is dissolved** — Badge's restyle package handles intentColorset itself via the ds2026 value-split.

**Why standalone (ratified orchestrator decision, resolving Deviations item 8 — historical, pre-supersession):** the wave-3 plan's cross-package serialisation table establishes `intentColorset` as feeding Badge (W3c-P1), Alert (W3c-P7) and IntentStripe; folding it into any one component's package couples unrelated components. **Measured consumer evidence (grep 2026-07-07):** the only *direct* importer of `styledIntentionalElement`/`variantIntentionalColors` today is `lib/components/Badge/Badge.css.ts` (Alert and IntentStripe currently render intents via the sprinkles `backgroundColour={intent}` path, not via intentColorset — the wave-3 serialisation additionally routes them through it). `focusOutline` is imported by **14+ files**: `lib/styles/elementReset.css.ts` plus Button, Tabs (Tab/TabPane), Calendar, SearchBar, Slider, Switch, CheckableBase, OptionList, OptionGrid, ToggleButtons, DateTimeField, Anchor. Either way the blast radius is multi-component — hence Builder `opus` and the widened Chromatic scope below.
...
Product-owner requirement (applies to all wave files). One shared checklist — do **not** duplicate per batch. Runnable by an agent in the MFE monorepo at `/Users/timamehro/grit/github.com/autoguru/mfe` (single `bun.lock`, **mfe is bun-managed** — `bun run overdrive:local`, not `yarn` — manual overdrive bumps, local-link via `.scripts/copy-overdrive.js`; the mfe repo-wide type-check is `bun run lint:tsgo`).

> **Two verified procedure corrections (2026-07-07, from the `cded6390` pre-publish A/B):**
> (a) **`.scripts/copy-overdrive.js` runs `yarn build` only (Babel) and never refreshes `.d.ts`.** A meaningful type-level check requires first running `yarn typeEmit` in overdrive and syncing the freshly-emitted `.d.ts` into the linked `node_modules/@autoguru/overdrive` — otherwise step 1's type-check silently type-checks against a **stale** `.d.ts`. (Filed as a future fix for `copy-overdrive.js` itself.)
> (b) **turbo's task cache is insensitive to `node_modules` swaps.** The repo-wide type-check MUST be invoked with `--force` — `./node_modules/.bin/turbo run lint:mfe --force` — or turbo replays a stale cached log and reports a **false PASS** even when the linked build changed.

1. **Pre-publish smoke (local link, before the Version Packages PR merges):**
   ```bash
   # in overdrive, on the package branch
   yarn build
   yarn typeEmit                 # required — copy-overdrive.js does not refresh .d.ts on its own
   # in the mfe monorepo
   cd /Users/timamehro/grit/github.com/autoguru/mfe
   bun run overdrive:local       # copies the local overdrive build in (.scripts/copy-overdrive.js) — bun-managed, not yarn
   bun run lint:tsgo             # repo-wide type-check — MUST be clean
   ./node_modules/.bin/turbo run lint:mfe --force   # --force required: turbo cache ignores node_modules swaps and will false-PASS without it
   ```
...
13. **Theme-bridge coupling breaks incomplete manual `theme.css` mocks in consumer tests (verified 2026-07-07, MFE pre-publish A/B at `cded6390`).** As of `C-theme-bridge` (deviation 12), `lib/themes/base/tokens.ts` imports `overdriveTokens` from `theme.css` **at module-eval time** (to alias `color.gamut` onto the legacy `colours.gamut` CSS vars). Any consumer test that manually mocks `@autoguru/overdrive/themes/theme.css` with a **partial** object (missing `overdriveTokens`) will now throw at import time instead of silently working, because the aliasing code dereferences `overdriveTokens.colours.gamut.*` eagerly. **One known MFE hit:** `packages/themes/fleetGuruTheme/__tests__/FleetGuruThemeProvider.test.tsx` (its mock lacks `overdriveTokens`). **Fix is one line** — either add `overdriveTokens` to the mock or switch it to `vi.importActual`/`jest.requireActual` (`importOriginal`) for the untouched parts. **Ship-note:** flag this to the MFE team alongside the release so they can patch the one test ahead of bumping.
14. **F-1/F-2 folded-purge packages were undocumented in this plan (found 2026-07-07, closed by this audit pass).** `af61bd14` ("purge safe legacy colour internals from folded components") landed two batches — **F-1** (`track-c-f1-controls-inputs.md`: CheckBox/Radio/Switch/CheckableBase/InputBase/Button safe internals) and **F-2** (`track-c-f2-nav-feedback.md`: Button/Tabs/Tooltip/Pagination/ProgressSpinner/Alert safe internals) — ahead of their Wave-3 fold, repointing everything **except** the intent-derived/theme-variant/public-API refs the folded package will itself resolve. Neither this file's §"Track C package map" (§1.9) nor master §9.2 listed them; §9.1's per-component table (this file, above) now carries an **F-1 ✅ / F-2 ✅** annotation on every affected row, and master §9.2 has a dedicated tracking row. **Tooltip is fully repointed by F-2** (its 2 refs were pure ramp/white values, no retained items) — its Wave-3 fold target (W3c-P3) is effectively superseded for colour purposes; the component still needs whatever non-colour restyle work W3c-P3 carries.
