...
| **TextLink** | `TextLink.css.ts` legacy `root`/`muted`; `TextLink.tsx` legacy `<Text colour>` | `typography.colour.link`(green600); JSX `colour` | `color.interactive.link`(green600) / **`color.link.*`** (landed — *not* `color.button.linkedText.*`, which never existed) | N | ✅ (interactive.link) | **W3c-P2** | **partly landed (AG-20713):** the new `variant` (linked-text) path consumes `color.link.*`; the legacy path deliberately stays on `typography.colour.link` — repointing it flips the default link colour green600 `#01C68C` → `#18856F`, which is major-only. Line refs were stale; see `docs/ds2026-specs/TextLink.md`. |
