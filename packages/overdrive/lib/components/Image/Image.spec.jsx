...
describe('<Image />', () => {
	...
	it.each([
		...
		['no', false],
		['yes', true],
	])('should match snapshot for unoptimised value of %s  when no ImageServerProvider is defined', (label, value) => {
		const { container } = render(
	...
	it.each([
		...
		['no', false],
		['yes', true],
	])('should match snapshot for unoptimised value of %s when ImageServerProvider is defined', (label, value) => {
		const { container } = render(
	...
	])('should match snapshot for sizes value of %s when ImageServerProvider is defined', (label, value) => {
		const { container } = render(
			<ImageServerProvider srcUrlMapper={srcUrlMapper}>
				<Image unoptimised={value} />,
			</ImageServerProvider>
